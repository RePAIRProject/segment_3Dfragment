...
void Evrot::cluster_assign() {
	...
	for (int i=0; i<mNumData; i++ ) {
		...
		mXrot.row(i).cwiseAbs().maxCoeff(&row, &col); // I changed
		max_index_col[i] = col;
	}
...
double Evrot::evqual(Eigen::MatrixXd& X) {
	...
	//Eigen::MatrixXd X2 = X.pow(yanivPow);// I changed https://eigen.tuxfamily.org/dox/group__CoeffwiseMathFunctions.html
	Eigen::MatrixXd X2 = X.array().square();// I changed https://eigen.tuxfamily.org/dox/group__CoeffwiseMathFunctions.html
...
double Evrot::evqualitygrad(Eigen::VectorXd& theta, int angle_index) {
	...
	for (int i=0; i<mNumData; i++ ) {
		...
		Y.row(i).cwiseAbs().maxCoeff(&row, &col);//https://eigen.tuxfamily.org/dox/group__CoeffwiseMathFunctions.html
