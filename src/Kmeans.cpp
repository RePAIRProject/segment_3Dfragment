...
std::vector<std::vector<int> > Kmeans::cluster(Eigen::MatrixXd& data, int ncentres) {
	...
	for (int i=0; i < ncentres; i++) {
		...
		do {
			...
			int randIndex = 0 + (std::rand() % ((ndata - 1) - 0 + 1)); // https://stackoverflow.com/questions/7560114/random-number-c-in-some-range
	...
	for (int n=0; n < niters; n++) {
		...
		double cdiff = (centres-old_centres).cwiseAbs().maxCoeff(); /// I changed
